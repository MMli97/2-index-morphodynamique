"""
Index des Formes Conceptuelles Dynamiques — Classifieur V2.1.2

Patch V2.1.2: Frontière F01/F03 uniquement.
Base: V2.1.1 (inchangée sauf F01, F03, F06).

Modifications:
  F01: Δ₂₃ ≤ 0.15 → Δ₂₃ ≤ 0.18
  F03: Δ₂₃ ≥ 0.25 → Δ₂₃ ≥ 0.23
  F06: Δ₂₃ ≤ 0.15 → Δ₂₃ ≤ 0.18 (parallèle à F01)

Tout le reste est IDENTIQUE à V2.1.1.
F04, F05, F08, F09: aucun changement.
Poids: aucun changement.

Justification:
  - T3 échouait principalement sur le corridor F01↔F03 (87.3%)
  - La zone grise [0.15, 0.25] était trop large (0.10 d'écart)
  - Réduction à [0.18, 0.23] (0.05 d'écart) — plus serré mais pas fermé
  - F04 reste tel quel (A1≥0.60, A5≤0.30) — le test V2.2 a montré
    que le relâcher dégrade les marges et les recouvrements
"""
...
def make_F01() -> Form:
    ...
    PATCH V2.1.2: Δ₂₃ ceiling 0.15 → 0.18
    """
    return Form("F01", "Équilibre différentiel", signature="Δ₂₃ ≤ 0.18, Δ₄₅ ≤ 0.30",
        constraints=[
            # Level 1: Gradient conditions (primary)
            Constraint("Δ₂₃ ≤ 0.18 (propagation ≈ intégration)",
                       lambda x, d: d["D23"] - 0.18, weight=2.5, level="gradient"),
        ...
        transitions=[
            Transition("F03", "Δ₂₃ franchit 0.18 ↑", "Propagation dépasse intégration"),
...
def make_F03() -> Form:
    ...
    PATCH V2.1.2: Δ₂₃ floor 0.25 → 0.23
    """
    return Form("F03", "Sur-couplage", signature="Δ₂₃ ≥ 0.23",
        constraints=[
            Constraint("Δ₂₃ ≥ 0.23 (propagation >> intégration)",
                       lambda x, d: 0.23 - d["D23"], weight=2.5, level="gradient"),
        ...
        transitions=[
            Transition("F01", "Δ₂₃ ↓ sous 0.18", "Intégration rattrape propagation"),
...
def make_F06() -> Form:
    ...
    PATCH V2.1.2: Δ₂₃ ceiling 0.15 → 0.18 (parallel to F01)
    """
    return Form("F06", "Inversion normative", signature="Δ₄₅ ≤ 0.05, A5 ≥ 0.50",
        constraints=[
            ...
            Constraint("Δ₂₃ ≤ 0.18 (propagation stabilisée)",
                       lambda x, d: d["D23"] - 0.18, weight=2.0, level="gradient"),
